...
const CheckoutForm2 = ({ bets, isCombinedBet }) => {
  ...
  const [totalAmount, setTotalAmount] = useState(0); // Stare pentru suma totală
  const [userId, setUserId] = useState(null); // Stare pentru ID-ul utilizatorului
  const [error, setError] = useState(null); // Stare pentru erori
  const [succeeded, setSucceeded] = useState(false); // Stare pentru succesul tranzacției
  ...
  // Calculează suma totală a pariurilor
  useEffect(() => {
  ...
  // Gestionează schimbarea valorilor din formular
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Loghează operațiunea
      const username = localStorage.getItem('username');
      const role = localStorage.getItem('role');
      await axios.post('https://localhost:8081/log-operation', {
        username,
        role,
        operation: 'create',
        table: 'pariu'
