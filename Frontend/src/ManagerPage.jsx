...
const ManagerPage = () => {
  ...
  const [loggedOperations, setLoggedOperations] = useState([]);
  const [adminUsers, setAdminUsers] = useState([]);
  const [allBets, setAllBets] = useState([]);
  const [invoices, setInvoices] = useState([]);
  const [selectedItem, setSelectedItem] = useState('operations'); // Elementul selectat implicit
  const [filter, setFilter] = useState({ username: '', operation: '', startDate: '', endDate: '' });
  const [eventFilter, setEventFilter] = useState({ eventType: '', startDate: '', endDate: '' });
  const [matchFilter, setMatchFilter] = useState({ eventType: '', startDate: '', endDate: '' });
  const [betFilter, setBetFilter] = useState({ category: '', startDate: '', endDate: '' });
  const [invoiceFilter, setInvoiceFilter] = useState({ name: '', startDate: '', endDate: '' });
  const [uniqueEventTypes, setUniqueEventTypes] = useState([]);
  const [uniqueNames, setUniqueNames] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);

  const [selectedReport, setSelectedReport] = useState('matchHistoryByEventType');
    ...
    return (
      <div className="pagination">
        ...
        <div className="export-buttons">
                <button className="export-button" onClick={() => exportToPDF(filteredEvents, 'Current Events')}>Export to PDF</button>
                <button className="export-button" onClick={() => exportToExcel(filteredEvents, 'Current Events')}>Export to Excel</button>
                <label className="export-button">
                  Import from Excel
                  <input type="file" className="file-input" accept=".xlsx, .xls" onChange={(e) => handleFileUpload(e, setCurrentEvents)} />
                </label>
              </div>
      </div>
    );
  ...
  // Grupăm facturile după dată și le numărăm
  ...
  const invoicesOverTime = {
    labels: Object.keys(invoicesByDate),
    datasets: [
      {
  ...
  const detailedLoggedOperations = {
    labels: filteredLoggedOperationsReport.map(item => `${item.Nume} - ${item.Operatie}`),
    datasets: [
      {
        label: 'Detailed Logged Operations',
        data: filteredLoggedOperationsReport.map(item => 1),
        backgroundColor: 'rgba(75, 192, 192, 0.2)',
        borderColor: 'rgba(75, 192, 192, 1)',
        borderWidth: 1,
      },
          ...
          <div>
            ...
            <div className="filter-controls">
              ...
              >
                ...
                <option value="detailedLoggedOperations">Detailed Logged Operations</option>
              </select>
            </div>
            ...
            {selectedReport === 'detailedLoggedOperations' && (
              <div>
                <h3>Detailed Logged Operations</h3>
                <div className="chart-container">
                  <Bar ref={chartRef} data={detailedLoggedOperations} options={options} />
                </div>
              </div>
            )}
